...
export function stats(): Promise<any> {
  var stats = {tickets: null, orders: null, pending: null, byShow: null, byDate: null};
  return db.query('select sum(price) as revenue, count(*) as count from nk2_tickets').then((res) => {
    ...
    return db.query('select sum(price) as revenue, count(*) as count from nk2_orders where status = "payment-pending"').then((res) => {
      stats.pending = res[0];
    });
